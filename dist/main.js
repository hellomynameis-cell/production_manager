...
const resetFormButton = document.getElementById('resetFormButton'); // NEW: Reset button
 ...
 */
function setupMachinesUI() {
    if (!machinesGridContainer)
        return;
    ...
    machineList.forEach(machine => {
        const machineOrdersDiv = document.querySelector(`#${machine.id} .machine-orders`);
        if (machineOrdersDiv) {
 ...
 */
function getLocationFromContainer(container) {
...
function handleDragEnd(evt) {
    ...
    const newOrderIdsInList = Array.from(targetList.children)
        .map(item => parseInt(item.dataset.orderId || '0'))
